(() => {
  ...
  function bindElements() {
    [
      ...
      "recentlyPlayedSection", "recentlyPlayedList", "profileTab", "profilePanel", "profileHero", "profileAvatar", "profileAvatarImage",
      "profileEyebrow", "profileTitle", "profileMeta", "profileSubtitle", "profileAuthBtn", "profileSignOutBtn", "profileShareBtn",
      ...
      "playlistDetailDescription", "playlistCoverImage", "playlistCoverFallback", "playlistPlayBtn",
      "playlistShuffleBtn", "playlistEditBtn", "playlistPublicToggle", "playlistShareBtn", "playlistDeleteBtn", "playlistTrackList",
      "playlistForm", "playlistNameInput", "playlistDescriptionInput", "playlistCoverInput", "playlistSaveBtn",
      "playlistSettingsModal", "playlistSettingsCloseBtn", "authModal", "authCloseBtn", "authDialogTitle", "authDialogCopy", "authGoogleBtn",
  ...
  function bindEvents() {
    ...
    if (els.playlistEditBtn) els.playlistEditBtn.addEventListener("click", openPlaylistSettings);
    ...
    if (els.playlistSettingsCloseBtn) els.playlistSettingsCloseBtn.addEventListener("click", closePlaylistSettings);
    if (els.playlistSettingsModal) {
      els.playlistSettingsModal.addEventListener("click", (event) => {
        if (event.target === els.playlistSettingsModal) closePlaylistSettings();
      });
    }
  ...
  function normalizePlaylistRows(rows) {
    ...
    if (els.profileMeta) els.profileMeta.textContent = state.publicView ? `Public profile \u2022 ${(state.publicView.playlists || []).length} playlists` : "Profile \u2022 0 playlists";
  ...
  function updateWelcomeCopy(name) {
    if (state.user) {
      ...
      if (els.homeWelcomeCopy) {
        els.homeWelcomeCopy.textContent = "Your recent plays and playlists stay synced to this account.";
      }
      if (els.authOpenBtn) els.authOpenBtn.textContent = "My Profile";
      return;
    }

    if (els.welcomeBanner) els.welcomeBanner.textContent = "Guest mode";
    if (els.homeWelcomeTitle) els.homeWelcomeTitle.textContent = "A library-first player built for deep listening.";
    if (els.homeWelcomeCopy) {
      els.homeWelcomeCopy.textContent = "Search tracks, jump into quick picks, and move between albums, singles, and playlists without losing the flow.";
    }
    if (els.authOpenBtn) els.authOpenBtn.textContent = "Continue";
  }
  ...
  function renderProfileView() {
    ...
    const publicPlaylistCount = (playlists || []).filter((playlist) => playlist.is_public).length;

    if (els.profileEyebrow) {
      els.profileEyebrow.textContent = publicView ? "Public Profile" : (state.user ? "Profile" : "Account");
    }
    if (els.profileTitle) {
    ...
    if (els.profileMeta) {
      if (publicView && profile) {
        els.profileMeta.textContent = `${(playlists || []).length} public playlist${(playlists || []).length === 1 ? "" : "s"} \u2022 Puresound profile`;
      } else if (profile) {
        els.profileMeta.textContent = `${(playlists || []).length} playlist${(playlists || []).length === 1 ? "" : "s"} \u2022 ${publicPlaylistCount} public`;
      } else {
        els.profileMeta.textContent = "Profile \u2022 Sign in with Google or GitHub";
      }
    }
    if (els.profileSubtitle) {
      if (publicView && profile) {
        els.profileSubtitle.textContent = profile.bio || "Public playlists and listening identity on Puresound.";
      } else if (profile) {
        els.profileSubtitle.textContent = profile.bio || "Build playlists, save recent plays, and shape your public profile.";
      } else {
    ...
    if (els.profileHero) els.profileHero.classList.toggle("hidden", showPlaylistPage);
  ...
  function renderPlaylistList(playlists, publicView) {
    ...
    playlists.forEach((playlist) => {
      ...
      const itemCount = (playlist.playlist_items || []).length;
      const isActive = playlist.id === state.selectedPlaylistId;
      const card = document.createElement("article");
      card.className = "release-card playlist-release-card" + (isActive ? " is-active" : "");
      if (coverUrl) {
      ...
      button.setAttribute("aria-label", `Open playlist ${playlist.name}`);
      ...
      if (coverUrl) {
        ...
        image.src = coverUrl;
        image.alt = `${playlist.name} cover art`;
        image.loading = "lazy";
        art.append(image);
      } else {
        const fallback = document.createElement("div");
        fallback.className = "release-card-fallback";
        fallback.textContent = initialsFor(playlist.name || "Playlist");
        art.append(fallback);
      }
      ...
      status.className = "release-card-status playlist-card-status";
      ...
      sub.textContent = playlist.description || (playlist.is_public ? "Public playlist" : "Private playlist");

      const meta = document.createElement("div");
      meta.className = "release-meta-row";
      meta.textContent = `${itemCount} ${itemCount === 1 ? "track" : "tracks"}${playlist.is_public ? " \u2022 public" : " \u2022 private"}`;
  ...
  function renderPlaylistDetail() {
    ...
    if (els.playlistDetailMeta) {
      els.playlistDetailMeta.textContent = `${playlistItems.length} ${playlistItems.length === 1 ? "track" : "tracks"}${playlist.is_public ? " \u2022 public" : " \u2022 private"}`;
    }
    if (els.playlistDetailDescription) {
      els.playlistDetailDescription.textContent = playlist.description || "Curated on Puresound.";
    }
    ...
    if (state.app && typeof state.app.applyVisualAccent === "function") {
      state.app.applyVisualAccent(els.playlistPage, coverUrl, `${playlist.name} playlist`);
    }

    if (els.playlistEditBtn) els.playlistEditBtn.classList.toggle("hidden", !canEdit);
    ...
    playlistItems.forEach((item, index) => {
      ...
      row.className = "playlist-item-row" + (currentTrack && record.trackId === currentTrack.id ? " is-active" : "");
  ...
  function openPlaylistSettings() {
    const playlist = getSelectedPlaylist();
    const publicView = Boolean(state.publicView && (state.route.profile || state.route.playlist));
    if (!playlist || publicView || !state.user || playlist.user_id !== state.user.id) return;
    toggleDialog(els.playlistSettingsModal, true);
  }

  function closePlaylistSettings() {
    toggleDialog(els.playlistSettingsModal, false);
  }
