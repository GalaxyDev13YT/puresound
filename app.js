...
function bindElements() {
  [
    ...
    "searchResultsSection", "searchTrackList", "albumList", "libraryPagination", "libraryEmpty", "libraryPanel", "releaseGridView", "settingsPanel", "profilePanel", "cacheToggle", "clearCacheBtn", "cacheCount",
...
function filterAndRenderReleases() {
  ...
  if (!query) {
    ...
    state.filteredReleases = state.releases.slice();
  } else {
    state.filteredTracks = getTopMatchingTracks(query).slice(0, SEARCH_TRACK_LIMIT);
    state.filteredReleases = getTopMatchingReleases(query).slice(0, SEARCH_RELEASE_LIMIT);
  }
  state.currentPage = 1;
  renderSearchTracks();
  renderReleaseGrid();
...
function getTopMatchingTracks(query) {
  const scored = [];
  state.releases.forEach((release) => {
...
function getTopMatchingReleases(query) {
