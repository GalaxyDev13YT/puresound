...
function bindElements() {
  [
    ...
    "albumList", "libraryEmpty", "libraryPanel", "releaseGridView", "settingsPanel", "cacheToggle", "clearCacheBtn", "cacheCount",
...
function bindEvents() {
  ...
  els.audio.addEventListener("waiting", () => {
    if (state.currentTrackId) setStatus("Buffering audioâ€¦");
  });
...
async function loadLibrary(options = {}) {
  ...
  if (state.cacheEnabled && !forceRefresh) {
    ...
    if (cached) {
      ...
      syncReleaseState();
      setStatus("Loaded cached library. Refreshing in backgroundâ€¦");
      toggleSetup(false);
      renderAll();
    }
  }
  ...
  setLoading(true, "Loading libraryâ€¦");
...
async function fetchRemoteLibrary(options = {}) {
  ...
  await runWithConcurrency(rootFolders, FOLDER_SCAN_CONCURRENCY, async (folder) => {
    ...
    if (completed === rootFolders.length || completed % 3 === 0) {
      setLoading(true, `Scanning releases ${completed}/${rootFolders.length || 1}â€¦`);
    }
  });
...
function renderReleaseGrid() {
  ...
  state.filteredReleases.forEach((release) => {
    ...
    meta.textContent = `${release.tracks.length} ${release.tracks.length === 1 ? "track" : "tracks"}${release.source === "local" ? " â€¢ local" : ""}`;
...
function renderReleaseDetail(release) {
  ...
  els.detailStats.textContent = `By ${release.artist}${release.source === "local" ? " â€¢ local files" : ""}`;
  ...
  release.tracks.forEach((track, index) => {
    ...
    sub.textContent = `${track.artist} â€¢ ${release.title}`;
...
function applyTrackSource(track, candidateIndex) {
  ...
  setStatus(candidateIndex > 0 ? `Trying backup source ${candidateIndex + 1}/${candidates.length} for ${track.title}â€¦` : `Loading ${track.title}â€¦`);
}

function refreshNowPlaying(track) {
  els.nowTitle.textContent = track ? track.title : "Nothing queued";
  els.nowSub.textContent = track ? `${track.artist} â€¢ ${track.album}` : "Choose a track to begin";
  if (!track) {
...
async function downloadSelectedRelease() {
  ...
  setStatus(`Starting ${release.tracks.length} download${release.tracks.length === 1 ? "" : "s"}â€¦`);
...
function setLoading(active, message = "Loadingâ€¦") {
...
function formatTrackMeta(track) {
  ...
  return `${extension || "AUDIO"} â€¢ ${track.sizeLabel}`;
}
...
function tryNextAudioCandidate(reason = "") {
  ...
  if (nextIndex < track.urlCandidates.length) {
    setStatus(reason ? `${reason}. Trying another sourceâ€¦` : `Trying another source for ${track.title}â€¦`);
